...
def _analyze_single_view(df: pd.DataFrame, view_id: str, view_name: str) -> tuple:
    """
    Analyze a single view's data.
    
    Returns:
        Tuple of (column_stats, trends, outliers, correlations)
    """
    ...
    return column_stats, trends, outliers, correlations
...
async def analyze_data_node(state: SummaryAgentState) -> Dict[str, Any]:
    ...
    Supports multiple views by analyzing each view separately.
    """
    try:
        # Check for multiple views first
        views_data = state.get("views_data", {})
        view_data = state.get("view_data")  # Backward compatibility
        
        # If we have multiple views, analyze each one
        if views_data:
            all_column_stats = {}
            all_trends = []
            all_outliers = []
            ...
            for view_id, v_data in views_data.items():
                ...
                view_name = views_metadata.get(view_id, {}).get("name", view_id)
                logger.info(f"Analyzing view: {view_name} ({view_id})")
                
                data_rows = v_data.get("data", [])
                columns = v_data.get("columns", [])
                
                if not data_rows or not columns:
                    logger.warning(f"View {view_id} has no data")
                    continue
                
                try:
                    df = pd.DataFrame(data_rows, columns=columns)
                except Exception as e:
                    logger.error(f"Error creating DataFrame for view {view_id}: {e}")
                    continue
                
                # Analyze this view
                view_stats, view_trends, view_outliers, view_correlations = _analyze_single_view(df, view_id, view_name)
                
                # Aggregate results
                all_column_stats[view_id] = view_stats
                all_trends.extend(view_trends)
                all_outliers.extend(view_outliers)
                if view_correlations:
                    all_correlations[view_id] = view_correlations
            ...
            return {
                **state,
                ...
                "current_thought": f"Analyzed {len(views_data)} view(s), found {len(all_trends)} trends and {len(all_outliers)} outlier groups"
            }
        
        # Fallback to single view (backward compatibility)
        if not view_data:
            return {
                **state,
        ...
        # Analyze single view
        column_stats, trends, outliers, correlations = _analyze_single_view(df, "single_view", "View")
        ...
        return {
            **state,
            ...
            "current_thought": f"Analyzed {len(df.select_dtypes(include=[np.number]).columns)} numeric columns, found {len(trends)} trends and {len(outliers)} columns with outliers"
        }
