...
def _detect_and_apply_date_functions(query_draft: Dict[str, Any], user_query: str, enriched_schema: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    for field in fields:
        ...
        if is_date_field and not field_function and "calculation" not in field:
            ...
            logger.warning(
                f"🔧 AUTO-FIX: Added '{detected_function}' function to date field '{field_caption}' "
...
def _detect_and_apply_count_functions(query_draft: Dict[str, Any], user_query: str, enriched_schema: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # This is the pattern: "how many customers" → field without function
    if len(fields) == 1:
        field = fields[0]
        field_caption = field.get("fieldCaption", "")
        ...
        if not has_function and field_caption:
            ...
            logger.warning(
                f"🔧 AUTO-FIX: Added 'COUNTD' function to field '{field_caption}' "
...
def _detect_and_apply_context_filters(query_draft: Dict[str, Any], user_query: str) -> Dict[str, Any]:
    ...
    if not has_context and len(filters) == 2:
        ...
        if filter_type in ["SET", "TOP", "QUANTITATIVE_NUMERICAL", "DATE"]:
            ...
            logger.warning(
                f"🔧 AUTO-FIX: Applied context:true to first filter (type: {filter_type}) "
        ...
        for i, f in enumerate(filters):
            ...
            if filter_type in ["SET", "DATE"] and not f.get("context"):
                f["context"] = True
                logger.warning(
                    f"🔧 AUTO-FIX: Applied context:true to {filter_type} filter at position {i} "
                    f"due to hierarchical pattern detected in: '{user_query}'"
                )
                modified = True
                break
        ...
        if not modified:
            ...
            if len(top_filters) >= 2:
                ...
                logger.warning(
                    f"🔧 AUTO-FIX: Applied context:true to TOP filter with howMany={first_top.get('howMany')} "
                    f"due to hierarchical pattern detected in: '{user_query}'"
                )
                modified = True
...
def _adjust_calculated_field_names(query_draft: Dict[str, Any], enriched_schema: Optional[Dict[str, Any]] = None, schema: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    for field in fields:
        ...
        if field_lower in existing_fields:
            ...
            if new_name:
                logger.warning(
                    f"🔧 AUTO-FIX: Renamed calculated field '{field_caption}' to '{new_name}' "
...
def _remove_fieldcaption_from_calculated_filters(query_draft: Dict[str, Any]) -> Dict[str, Any]:
    """
    Remove fieldCaption from filter field objects that have a calculation key.
    
    In Tableau VizQL API, filter fields with calculations should not include fieldCaption.
    This function cleans up the query after LLM generation to ensure compliance.
    
    Args:
        query_draft: The query generated by the LLM
        
    Returns:
        Modified query_draft with fieldCaption removed from calculated filter fields
    """
    if not query_draft or not isinstance(query_draft, dict):
        return query_draft
    
    query = query_draft.get("query", {})
    if not query:
        return query_draft
    ...
    for filter_obj in filters:
        ...
        if isinstance(filter_field, dict) and "calculation" in filter_field:
            # Remove fieldCaption if it exists
            if "fieldCaption" in filter_field:
                del filter_field["fieldCaption"]
                modified = True
                logger.debug(
                    f"Removed fieldCaption from calculated filter field. "
                    f"Filter now: {json.dumps(filter_field, indent=2)}"
                )
